...
// CORS Configuration (For Vercel Deployment - Allowing all origins)
app.use(
  cors({
    origin: true, // Allow all origins - for quick testing
...
...`;
    ...
    if (nameLine) {
      ...
      if (playlistName.length > 50)
        playlistName = playlistName.substring(0, 50) + '...'; // Truncate long names
    }

    // Parse songs
    const songs = lines.filter((line) => line.match(/^.+\s-\s.+$/)).slice(0, 10);
      ...
      songs.map(async (song, index) => {
        try {
          ...
          const validVideo = ytResponse.data.items.find((item) => {
            ...
            return (
              !title.includes('cover') && !title.includes('live') && item.id.videoId
            );
          });

          const videoId =
            validVideo?.id?.videoId || ytResponse.data.items[0]?.id?.videoId;
